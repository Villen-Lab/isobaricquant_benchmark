...
plot_grid(p_scores_SPS, p_scores_RTS, p_scores_MS2, labels = "AUTO", ncol = 2, rel_heights = c(4,3))
ggsave2("figures/fig_S4.pdf", width = 20, height = 12.5)
...
QC_data_lm[, QC_par_ext := factor(QC_par, levels = sapply(QC_par_list, function(x) x[1] )[14:1],
...
p_point_slope <- ggplot(QC_data_lm[type=="ratio 1:20"], aes(x=condition, y=QC_par_ext, color=slope, size=R_squared)) +
  geom_point() + geom_hline(yintercept = c(2.5, 10.5), linetype = "dashed") + xlab("") + ylab("") +
...
p_ratio_cor_peptideIntensityScore <- ggplot(data_COy_MS3, aes(x=peptideIntensityScore, y=ratio_20_m)) +
  ...
  facet_grid(rows = vars(condition)) + scale_fill_viridis("density") + theme_minimal_grid() + ylab("peptide mean ratio 20:1")
...
count_1 <- data_COy_MS3[, .N, keyby = c("condition", "ms1PrecSIgnal_eq1")][, .(ms1_1 = paste0(round(N[2]/sum(N), 3)*100, "% excluded")),
                                                                           keyby = c("condition")]
p_ratio_cor_ms1PrecSIgnal <- ggplot(data_COy_MS3[ms1PrecSIgnal!=1], aes(x=ms1PrecSIgnal, y=ratio_20_m)) +
  ...
  facet_grid(cols = vars(condition)) + scale_fill_viridis("density") + theme_minimal_grid() + ylab("peptide mean ratio 20:1")


count_1_shown <- data_COy_MS3[, .N, keyby = c("condition", "ms1PrecSIgnal_eq1")][, .(ms1_1 = paste0(round(N[2]/sum(N), 3)*100, "% shown")), keyby = c("condition")]
p_ratio_density_ms1PrecSIgnal_1 <- ggplot(data_COy_MS3[ms1PrecSIgnal==1], aes(x=ratio_20_m)) +
  geom_density() + geom_vline(xintercept = 20, linetype = "dashed") + xlab("peptide mean ratio 20:1\n(only showing ms1PrecSignal = 1)") +
...
This code block generates yeast peptide mean ratios plots (Fig 4d).
...
# == peptide mean ratio, separated by methods ==
...
p_pep_ratio_peptideIntensityScore <- ggplot(QC_pep_ratio, aes(x=condition, y=ratio, fill=type)) +
  ...
  scale_fill_viridis_d("peptideIntensityScore", option = "B") + xlab("") + ylab("peptide mean ratio 20:1") +
...
p_pep_ratio_peptideTopXIntensityFromTotalScore <- ggplot(QC_pep_ratio, aes(x=condition, y=ratio, fill=type)) +
  geom_boxplot(outlier.alpha = 0.2, color = "grey", lwd=0.5, fatten=1) +
  geom_text(data=lab, aes(x=condition, y=y+c(1.5, 3.5, 4), label=N), position = position_dodge(0.75), size = 3) +
  scale_fill_viridis_d("peptideTopXIntensityFromTotalScore", option = "B") + xlab("") + ylab("peptide mean ratio 20:1") +
  theme_minimal_hgrid() + theme(legend.position = "top") + guides(fill = guide_legend(title.position="top"))
...
p_pep_ratio_msnTotalSignal_log10 <- ggplot(QC_pep_ratio, aes(x=condition, y=ratio, fill=type)) +
  ...
  scale_fill_viridis_d("msnTotalSignal", option = "B") + xlab("") + ylab("peptide mean ratio 20:1") +
  theme_minimal_hgrid() + theme(legend.position = "top") + guides(fill = guide_legend(title.position="top"))

```

## Protein CV and median ratio (min 2 peptides)
...
p_min2_CV_msnTotalSignal <- ggplot(prot_min2_msnT[], aes(x=condition, y=CV, fill=type)) +
  ...
  theme_minimal_hgrid() + theme_yaxis_percentage + theme(legend.position = "bottom") + guides(fill = guide_legend(title.position="top"))
...
```{r fig_4, fig.width=15, fig.height=15}

# ##Fig 4: HYpro16 MS2/3 filter options (IQ vs FP / MQ) - wide version
...
# ggsave2("figures/fig_4.pdf", width = 20, height = 10)
...
p_fig4b <- plot_grid(p_ratio_cor_msnTotalSignal_log10, p_pep_ratio_msnTotalSignal_log10,
                     ...
                     p_ratio_cor_peptideTopXIntensityFromTotalScore, p_pep_ratio_peptideTopXIntensityFromTotalScore,
                     labels = c("C", "D"), ncol = 2)
p_fig4l <- plot_grid(NULL, p_point_slope_legend, nrow = 1, rel_widths = c(0.1, 0.9))
p_fig4a <- plot_grid(p_fig4l, p_point_slope + theme(legend.position = "none"), p_ratio_density_ms1PrecSIgnal_1,
                     labels = c("A", "", "B"), rel_heights = c(0.2, 1.8, 1), ncol = 1)
p_fig4ab <- plot_grid(p_fig4a, p_fig4b, rel_widths = c(1, 2))
p_fig4c <- plot_grid(p_min2_CV_msnTotalSignal, p_min2_CV_peptideIntensityScore, p_min2_CV_peptideTopXIntensityFromTotalScore,
                     nrow = 1, labels = c("E"), align = "h", axis = "tb")
plot_grid(p_fig4ab, p_fig4c, rel_heights = c(3, 1), nrow = 2)
ggsave2("figures/fig_4.pdf", width = 15, height = 15)
...
ggsave2("figures/fig_S6.pdf", width = 15, height = 10)
...
ggsave2("figures/fig_S1.pdf", width = 15, height = 10)
...
ggsave2("figures/fig_2.pdf", width = 10, height = 5)
...
ggsave2("figures/fig_S2.pdf", width = 15, height = 15)
...
p_sub <- ggplot(upset_dt, aes(x=cond, y=variable, alpha=value)) +
  geom_point(size=4) + facet_grid(cols = vars(condition)) +
...
ggsave2("figures/fig_S5.pdf", width = 10, height = 10)
